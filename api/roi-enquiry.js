...
function buildPdfBuffer(data) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(12).text(`Submitted: ${data.submittedAt || new Date().toISOString()}`)
      ...
      doc.fontSize(12).text(`Total Annual Value: ${data.calculator?.outputs?.totalAnnualValue ?? ""}`)
      doc.text(`REE Annual Fee: ${data.calculator?.outputs?.reeAnnualFee ?? ""}`)
      doc.text(`Net Annual Benefit: ${data.calculator?.outputs?.netAnnualBenefit ?? ""}`)
